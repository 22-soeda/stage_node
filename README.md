...
- **GUIモニター**: PySide6 + matplotlib によるモニターを同梱しており、ZMQ経由の接続・矢印キー相対移動、および3Dプレビュー（下向き三角錐の先端＝ステージ座標）で現在位置を確認できます。
...
├── monitor.py              # ステータス表示・接続・矢印移動・3Dプレビュー (PySide6 + matplotlib)
...
- `matplotlib` : モニターの3Dプレビュー用（`monitor.py`）